...
exports.placeOrder = async (req, res) => {
  try {
    ...
    console.log("shippingAddresssId",shippingAdrsId);
    ...
    const orderData = {
      userId,
      orderId: new mongoose.Types.ObjectId(), // You can implement a function to generate a unique order number
...
exports.orderDetails = async (req,res)=> {
  try {
    ...
    const orders = await ordersCollection.findById(orderId); // Find the order by its ID
    ...
    return res.render("user/orderDetails", { orders });
} catch (error) {
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    // Find the order by its orderId and update the status to 'cancelled' along with the cancellation reason
